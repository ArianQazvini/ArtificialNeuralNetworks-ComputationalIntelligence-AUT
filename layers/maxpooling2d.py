...
class MaxPool2D:
    ...
    def target_shape(self, input_shape):
        ...
        H = int((input_shape[0]-self.kernel_size[0])/self.stride[0])+1
        W = int((input_shape[1]-self.kernel_size[1])/self.stride[1])+1
        return H, W
    ...
    def backward(self, dZ, A_prev):
        ...
        strideh, stridew = self.stride
        batch_size, H_prev, W_prev, C_prev = A_prev.shape
        # batch_size, H, W, C = (batch_size,self.target_shape((H_prev,W_prev))[0],self.target_shape((H_prev,W_prev))[1],C_prev)
